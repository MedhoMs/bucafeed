<script setup>
    ...
    const handleJoin = async () => {
        ...
        if (!token.value) {
            alert(t.value.events?.loginRequired || 'Debes iniciar sesión para unirte a eventos');
            return;
        }
        try {
    ...
    <div class="min-h-screen">
        ...
        <main class="lg:pl-75 pt-20 lg:pt-0 flex flex-col min-h-screen">
            ...
            <div class="px-6 lg:px-14 pt-6" v-if="isUnverified">
                <UnverifiedBanner 
                    compact 
                    :message="t.events?.unverifiedBanner || 'Puedes ver la información del evento, pero no podrás inscribirte hasta que tu centro verifique tu identidad.'"
                />
            </div>
            <PageHeader 
            ...
            >
                <template #headerActions>
                    <button @click="isUnverified ? null : handleJoin()" 
                        ...
                        {{ isUnverified ? (t.events?.blockedUnverified || 'Bloqueado (Sin verificar)') : (eventDetails?.joined ? (t.events?.joinedStatus || 'Anotado / Dentro') : (t.events?.participate || 'Participar')) }}
                    </button>
                </template>
            </PageHeader>
            ...
            <section class="flex flex-col gap-8 text-white w-full max-w-screen-2xl mx-auto px-6 lg:px-14 mb-20">
                <div class="flex flex-col sm:flex-row gap-8 sm:gap-14 w-fit mt-4">
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="font-bold text-xs">{{ t.events?.educationalCenter || 'Institución educativa' }}</p>
                            <p>{{ eventDetails?.center_name || (t.events?.unknown || 'Desconocido') }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-2">
                        ...
                        <div>
                            <p class="font-bold text-xs">{{ t.events?.timeRange || 'Hora inicio - Hora fin' }}</p>
                ...
                <div class="flex flex-col lg:flex-row gap-8 bg-white/5 border border-white/10 rounded-3xl p-6 md:p-10">
                    ...
                    <div class="flex flex-col gap-8 flex-1">
                        <div class="flex flex-col gap-4">
                            <h2 class="text-3xl font-black uppercase tracking-tighter">{{ eventDetails?.title || (t.events?.detailsTitle || 'Detalles del Evento') }}</h2>
                            <p class="max-w-3xl text-white/80 whitespace-pre-line text-sm md:text-base leading-relaxed">{{ eventDetails?.description || (t.events?.noDescription || 'No hay descripción disponible para este evento.') }}</p>
                        </div>
                        <div class="flex flex-col sm:flex-row gap-8 sm:gap-12">
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="font-bold text-xs">{{ t.events?.participants || 'Participantes' }}</p>
                                    <p>{{ eventDetails?.participants_count || 0 }} {{ t.events?.admitted || 'admitidos' }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                ...
                                <div>
                                    <p class="font-bold text-xs">{{ t.events?.location || 'Ubicación' }}</p>
                                    <p>{{ eventDetails?.location || (t.events?.noLocation || 'Ubicación no disponible') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <button @click="downloadPDF" class="flex items-center gap-2 mt-10 self-center w-fit bg-accent-normal hover:bg-accent-normal-hover text-white px-8 py-3.5 cursor-pointer rounded-full duration-300 shadow-lg font-black tracking-widest text-xl">
                    ...
                    {{ t.events?.readBrochure || 'Leer Folleto del Evento' }}
                </button>
