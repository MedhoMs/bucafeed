...
<template>
    <main class="relative flex flex-col justify-center items-center min-h-screen bg-black/50">
        ...
        <span id="formErrWarning"
            class="absolute opacity-0 top-5 p-2 bg-red-300/80 border-2 rounded-xl border-red-500/80 transition-all duration-300">
        ...
        <div class="flex justify-center items-center mb-37.5" id="form-container">
            <div class="flex flex-col justify-center items-center h-100 w-100 p-2.5 text-white rounded-bl-xl rounded-tl-xl"
                id="side-panel">
                <p class="text-center text-[29px] font-bold mb-7.5 [text-shadow:-2px_2px_1px_black]" id="welcome">{{
                    t.register.welcome }}<span class="text-[#a0c4d4]">{{ t.nav.website }}</span></p>
                <img class="w-22.5 h-25" src="/src/assets/logo/logoTelamon.png" alt="">
                <p class="text-center text-[20px] font-bold mt-7.5 [text-shadow:-2px_2px_1px_black]" id="eslogan">{{
                    t.register.eslogan }}{{ t.nav.website }}</p>
            </div>
            <form id="registerForm"
                class="relative flex flex-col justify-center h-100 w-100 p-2.5 pl-5 pr-5 bg-white rounded-br-xl rounded-tr-xl"
                method="post">

                <section id="allRolesForm" class="forms flex flex-col mb-32">
                    <label class="font-bold mt-7.5" for="username-register-form" id="username-register-label">{{
                        t.register.email }}</label>
                    <span class=" text-xs">{{ t.register.emailSpan }}</span>
                    <input type="text" class="allRolesInput outline-hidden border-b border-black mb-7.5 p-0.5 text-xl"
                        maxlength="50" autocomplete="off" id="email-register-form" name="email-register-form"
                        :placeholder="t.register.placeholderEmail" required>
                    <p hidden class="absolute top-27.5 left-33.75 font-semibold">Email inválido</p>
                    <label class="font-bold" for="password-register-form" id="password-register-label">{{
                        t.register.password }}</label>
                    <input type="password"
                        class="allRolesInput outline-hidden border-b border-black mb-7.5 p-0.5 text-xl" maxlength="20"
                        autocomplete="off" id="password-register-form" name="password-register-form"
                        :placeholder="t.register.placeholderPassword" required>
                ...
                <section id="studentTeacheEuForm" class="forms flex-col mb-20 hidden">
                    ...
                    <input type="text"
                        class="studentTeacheEuInput outline-hidden border-b border-black mb-7.5 p-0.5 text-xl"
                        maxlength="50" autocomplete="off" id="name-register-form" name="name-register-form"
                        :placeholder="t.register.placeholderEmail" required></input>
                    <p hidden class="absolute top-22.5 left-7.5 font-semibold">El nombre debe tener entre 2 y 12 letras
                    </p>
                    <label class="font-bold" for="student-name" id="studentName">Apellidos</label>
                    <input type="text"
                        class="studentTeacheEuInput outline-hidden border-b border-black mb-7.5 p-0.5 text-xl"
                        maxlength="50" autocomplete="off" id="surname-register-form" name="surname-register-form"
                        :placeholder="t.register.placeholderEmail" required></input>
                    ...
                    <input type="text"
                        class="studentTeacheEuInput outline-hidden border-b border-black mb-7.5 p-0.5 text-xl"
                        maxlength="50" autocomplete="off" id="dni-register-form" name="dni-register-form"
                        :placeholder="t.register.placeholderEmail" required></input>
                ...
                <section id="validateEmailForm" class="forms hidden flex-col items-center mb-16">
                    <p id="validateEmailText" class="mb-5 text-center font-bold">Hemos enviado un código de verificación
                        a tu correo</p>
                    <input type="text"
                        class="validateEmailInput outline-hidden border-b border-black mb-7.5 p-0.5 w-32 text-center text-xl"
                        maxlength="6" autocomplete="off" id="code-register-form" name="code-register-form"
                        required></input>
                </section>
                ...
                <ButtonForm id="nextButton" value="Siguiente" class="absolute bottom-15 right-12.5 border rounded-lg">
                </ButtonForm>

                <RouterLink to="/login"
                    class="absolute bottom-5 right-16 flex justify-center items-center text-[15px] gap-1 text-[#4a4a4a] font-bold transition-all duration-200 ease-in-out hover:brightness-200"
                    id="redirect-login">{{ t.register.haveAccount }}<svg width="20" height="20" viewBox="0 0 24 24"
