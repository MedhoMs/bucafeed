<script setup>
/**
 * EventsTab.vue - Gestión de Eventos para administradores de centros
 */
...
const filteredEvents = computed(() => {
    if (!search.value.trim()) return props.events
    ...
    <div class="space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-700">
        ...
        <div v-if="filteredEvents.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-10">
            <EventCard 
                ...
                :event="event" 
                mode="manage"
                @edit="emit('openModal', 'edit_event', event)"
                @delete="emit('deleteItem', 'event', event)"
            />
        </div>
