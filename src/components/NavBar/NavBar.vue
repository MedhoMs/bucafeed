    ...
    <Transition
        ...
        <nav v-if="menu || true" v-show="menu || isDesktop" class="fixed top-0 bottom-0 left-0 flex flex-col w-75 px-5 nav-sidebar shrink-0 overflow-y-auto overflow-x-hidden">
            ...
            <section class="flex flex-col h-full">
                ...
                <NavBarLinks v-if="user?.role !== 'EU'" to="/foro" :title=t.nav.foro>
                    <template #icon>
                ...
                <NavBarLinks to="/publicaciones" :title="t.nav.publications || 'Publicaciones'">
                    <template #icon>
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M3 5a2 2 0 0 1 2 -2h14a2 2 0 0 1 2 2v14a2 2 0 0 1 -2 2h-14a2 2 0 0 1 -2 -2z M7 7h10 M7 12h10 M7 17h10"/></svg>
                    </template>
                </NavBarLinks>
                ...
                <NavBarLinks v-if="user?.role !== 'EU'" to="/meeting" :title=t.nav.meeting>
                    <template #icon>
                ...
                <NavBarLinks v-if="user?.role !== 'EU' || user?.is_legal_tutor" to="/private-chat" :title=t.nav.privateChat>
                    <template #icon>
