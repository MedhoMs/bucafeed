        ...
        <nav v-if="menu || true" v-show="menu || isDesktop" class="fixed top-0 left-0 flex flex-col w-75 h-screen px-5 nav-sidebar shrink-0 overflow-y-auto overflow-x-hidden">
            ...
            <section class="flex flex-col h-full">
                ...
                <Transition
                    ...
                    <div v-if="showPopup" @click.stop="showPopup = false" class="absolute right-10 bottom-24 bg-background border border-white/10 rounded-xl shadow-lg overflow-hidden">
                        <router-link class="flex gap-2.5 m-1 text-base items-center py-3 px-4 rounded-xl text-white no-underline transition-all duration-200 ease-in-out font-medium hover:bg-secondary-normal active:font-semibold" :to="'/profile/' + user?.id">
