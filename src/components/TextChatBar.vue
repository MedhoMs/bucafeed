<script setup>
    ...
    function onClickOutside(e) {
        const picker = document.querySelector('.custom-emoji-picker-container')
        const btn = emojiBtnRef.value
        if (showEmojiPicker.value && picker && !picker.contains(e.target) && btn && !btn.contains(e.target)) {
    ...
    <div class="flex my-3 w-full">
        <div class="flex items-center border border-white rounded-[20px] p-3.75 w-full relative">
            ...
            <textarea 
                ...
                id="chatBar" 
                rows="1"
                class="mx-2 w-full outline-hidden text-base border-none bg-transparent flex-1 resize-none text-[#e7e9ea] placeholder-[#8b98a5] placeholder-font-normal leading-relaxed" 
            ...
            <div class="flex items-center gap-1 shrink-0 relative">
                ...
                />
                <button ref="emojiBtnRef" @click="showEmojiPicker = !showEmojiPicker" type="button" class="emoji-toggle-btn hover:text-white transition-colors cursor-pointer text-white/50 hover:bg-[#152027] rounded-lg p-1">
