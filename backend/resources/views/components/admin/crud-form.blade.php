    ...
    >
        ...
        <div class="flex items-center justify-between mt-8">
            @if($showCancel)
                ...
                @else
                    <button type="button" class="px-8 py-3 rounded-2xl bg-white/5 hover:bg-white/10 text-white/60 hover:text-white border border-white/10 transition-all duration-300 text-sm font-bold uppercase tracking-widest flex items-center gap-2 cursor-pointer" data-bs-dismiss="modal" onclick="document.getElementById('default-modal').classList.add('hidden')">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M19 12H5M12 19l-7-7 7-7"/></svg>
                        {{ $cancelText }}
                    </button>
                @endif
            ...
            <div class="flex items-center gap-3">
                @if (($oper != 'destroy' && $oper != 'show') && (!$disabled || $oper == 'edit'))
                    <button type="submit" class="w-full md:w-auto bg-linear-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white px-8 py-3 rounded-xl font-bold transition-all duration-300 shadow-lg shadow-cyan-900/20 active:scale-95 flex items-center justify-center gap-2 cursor-pointer">
                ...
                @if ($oper == 'destroy' && empty($datos['exito']))
                    <button type="submit" class="px-6 py-2 rounded-xl bg-red-500/20 hover:bg-red-500/30 text-red-400 border border-red-500/30 transition-all duration-200 cursor-pointer">
                        {{ $deleteText }}
                    </button>
                @endif
            </div>
