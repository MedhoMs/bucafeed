...
<div class="relative inline-block text-left filter-dropdown">
    ...
    <div class="dropdown-menu hidden absolute {{ $align === 'right' ? 'right-0' : 'left-0' }} mt-2 w-56 rounded-2xl shadow-2xl bg-[#0f1922] border border-white/10 ring-1 ring-black ring-opacity-5 focus:outline-none z-[100] overflow-hidden">
        <div class="py-1 max-h-64 overflow-y-auto custom-scroll">
            ...
            @foreach($options as $value => $text)
                <a 
                    ...
                    data-load="section"
                    class="block px-4 py-3 text-sm text-white/50 hover:bg-[var(--admin-primary-soft)] hover:text-white transition-colors {{ $selected == $value ? 'bg-[var(--admin-primary)]/40 text-white font-bold' : '' }}"
                >
                    {{ $text }}
                </a>
            @endforeach
